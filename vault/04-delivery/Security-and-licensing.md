...
### Local engine threat notes (D4 additions)

Recorded 2026-09-24 with [ADR-008](../03-decisions/ADR-008-Extractor-core-and-youtube-phase.md). The D4 implementation must honor these in addition to the D2 table.

| Threat | Control |
| --- | --- |
| Extractor-declared request headers | Header names are an allowlist in shared code ([T-056](../06-tasks/T-056-Http-request-port.md)); `Cookie`, `Authorization`, and Google auth headers are refused until [T-018](../06-tasks/T-018-Cookie-lifecycle.md). Every extracted media, manifest, fragment, and player-JS URL passes `UrlPolicy` before the request. |
| Embedded JavaScript runtime | Evaluates only the bundled, hash-verified solver plus the fetched player JS as data input, never page HTML. Hard timeout and cancellation on every host; solver output never reaches logs. The web page runs it in a worker under a CSP with no remote script origin. |
| Signed media URLs and tokens | `googlevideo` URLs, visitor ids, and PO tokens are job-internal. Fixtures are synthesized; the recorder redacts; tests grep for real hosts. Evidence records counts, not URLs. |
| Fragment downloaders | Bounded fragment count and retries; AES-128 keys are fetched through the same policy and never logged; the temp file is discarded on cancel. |
...
| yt-dlp-ejs / JS runtime | EJS has Unlicense code and MIT/ISC bundled components; runtime has its own terms. Re-inspected 2026-09-24: [yt-dlp/ejs](https://github.com/yt-dlp/ejs) reports SPDX `Unlicense`; release `0.8.0` (2026-03-17) is the version yt-dlp `2026.08.19` vendors. Upstream's `_info.py` hashes are **SHA3-512** (`jsc/_builtin/ejs.py` uses `hashlib.sha3_512`), not SHA-512. | Bundled at [T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md) under `third_party/yt-dlp-ejs/0.8.0/`: the core/lib pair (unminified + minified), `UNLICENSE`, and `HASHES.sha512` with the four upstream SHA3-512 values; `:shared:core:verifyEjsBundle` fails the build on a missing or tampered file, and `EjsScripts.kt` is generated from the verified minified pair. No runtime script download. The two NPM wrapper variants (deno/bun) are not bundled and their hashes are recorded for reference. The version follows the yt-dlp pin. |
| Zipline (Cash App) — QuickJS binding | GitHub API reports SPDX `Apache-2.0`; release `1.27.0`, inspected 2026-09-24. Bundles QuickJS (MIT) for JVM, Android, and Kotlin/Native. | Embedded runtime for D4 ([T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md) spike, [T-071](../06-tasks/T-071-Js-runtime-desktop-android-ios.md)). Spike 2026-09-24: Zipline's QuickJS reports version **2021-03-27** (older than the 2025-04-26 yt-dlp-ejs recommends for speed; R-16), and it evaluated the bundled lib+core on the JVM (506 ms) and the iOS Simulator (60 ms) with the synthetic challenge input; the Android adapter compiles but its spike is not run here (no emulator). Keep the Apache-2.0 and QuickJS MIT notices; the runtime only evaluates the bundled solver and never page content. |
| Deno (desktop, optional) | MIT; a separate executable the user installs. | Only as an optional desktop fallback probed on `PATH` under `apps/desktop`, argument list, no shell string. Never bundled. |
