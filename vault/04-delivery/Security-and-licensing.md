...
| yt-dlp-ejs / JS runtime | EJS has Unlicense code and MIT/ISC bundled components; runtime has its own terms. Re-inspected 2026-09-24: [yt-dlp/ejs](https://github.com/yt-dlp/ejs) reports SPDX `Unlicense`; release `0.8.0` (2026-03-17) is the version yt-dlp `2026.08.19` vendors, with SHA-512 hashes in `yt_dlp/extractor/youtube/jsc/_builtin/vendor/_info.py`. | D4 ([ADR-008](../03-decisions/ADR-008-Extractor-core-and-youtube-phase.md), [T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md)) bundles the pinned release under `third_party/yt-dlp-ejs/` with `UNLICENSE` and the upstream hashes; the build verifies the hashes and fails on mismatch. No runtime script download. Review the bundled components' MIT/ISC notices when the files land. The version follows the yt-dlp pin. |
| Zipline (Cash App) — QuickJS binding | GitHub API reports SPDX `Apache-2.0`; latest release `1.27.0` (2026-04-02), inspected 2026-09-24. Bundles QuickJS (MIT) for JVM, Android, and Kotlin/Native. | Candidate embedded JavaScript runtime for D4 ([T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md) spike, [T-071](../06-tasks/T-071-Js-runtime-desktop-android-ios.md)). Keep the Apache-2.0 and QuickJS MIT notices; record the QuickJS version it ships, since yt-dlp-ejs recommends quickjs ≥ 2025-04-26 for speed. The runtime only evaluates the bundled solver; it never evaluates page content. |
