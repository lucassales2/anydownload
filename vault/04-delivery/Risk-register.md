...
| R-16 | The embedded JavaScript engine is too slow or fails on a host (Zipline's QuickJS may predate the version yt-dlp-ejs recommends; iOS binding unproven). **High** | Spike with timings per target before adapters; JavaScriptCore fallback on iOS, Deno on `PATH` fallback on desktop; JS-less client keeps working without any runtime. | [T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md), [T-071](../06-tasks/T-071-Js-runtime-desktop-android-ios.md) |
| R-17 | YouTube client, innertube, or challenge behavior changes faster than the pinned tag; a translated extractor breaks with no upstream to shell out to on iOS/web. **High** | Pin moves only at phase boundaries; opt-in live harness and `yt-dlp -J` oracle detect drift; typed errors name the engine revision; desktop CLI and Chaquopy stay for unmatched URLs. | [T-059](../06-tasks/T-059-Extractor-test-harness.md), [T-062](../06-tasks/T-062-Desktop-ytdlp-oracle.md), [T-074](../06-tasks/T-074-Phase-4-verification.md) |
| R-18 | A recorded fixture or log leaks a signed `googlevideo` URL, visitor id, or PO token into the public repo. **High** | Synthesized fixtures only; recorder redaction unit-tested; tests grep fixtures for real hosts; evidence records counts, never URLs. | T-059, [T-060](../06-tasks/T-060-Youtube-jsless-extractor.md); see R-14 |
| R-19 | "Equivalence" is read as full coverage while the generated count is near 1 of 1,751. **Medium** | The equivalence note shows the generated count and the honest statuses; README states single-file quality and the hidden-format count. | [T-055](../06-tasks/T-055-Port-manifest-and-equivalence-matrix.md), T-074 |
