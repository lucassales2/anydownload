...
| --- | --- | --- | --- | --- |
| F-01 | Submit a video/social-media URL supported by yt-dlp. | Single-URL flow on all four clients; metadata/title, unsupported/auth-required errors, source attribution. No universal site guarantee. | M1 / [T-010](../06-tasks/T-010-Remote-vertical-slice.md) | R, U, A |
...
| F-07 | Live job status, progress, speed, ETA; real-time UI updates. | Honest phase-aware progress, unknown totals supported, reconnect snapshot/event recovery. | M1–M2 / T-010, T-011 | U, A |
...
| F-11 | Completed file open/download links, history, record removal, optional file deletion, auto-clear. | Separate server completion, device transfer, history removal, and actual deletion; preserve related chapter/sidecar links; test retention semantics. | M2–M3 / T-014, [T-016](../06-tasks/T-016-Clips-chapters-SponsorBlock.md) | R, U, A |
| F-12 | Video/audio roots, temporary/state directories, custom-folder selection/creation, exclusions, default folder. | Equivalent bounded storage controls with normalized relative paths, permissions, and disk-space checks. No state/secret exposure through file serving. | M2–M3 / T-014, [T-021](../06-tasks/T-021-Self-host-operations.md) | R, U, A |
...
| F-18 | Upload/replace/delete browser-exported cookies and show active status. | Explicit authorized upload to trusted server; secret reference only in jobs; size/format validation, scoped access, deletion and redaction. No promise that cookies guarantee site access. | M3 / [T-018](../06-tasks/T-018-Cookie-lifecycle.md) | R, U, A |
| F-19 | Global yt-dlp JSON options via environment/file; watched reload; named multiple presets; file precedence; ordered layering and null clearing. | Document and test global → ordered presets → approved overrides; file values take documented precedence; immutable effective options per attempt. Cover safe rate/proxy/archive/metadata options. | M3 / T-017, T-021 | R, A, D |
| F-20 | Optional per-download free-form yt-dlp options (upstream warns of command-execution risk). | **Proposed security difference:** allowlisted, authenticated overrides only; no arbitrary executable options, paths, plugins, or untrusted proxy overrides. Owner approval required; unrestricted parity is not claimed. | M3 / T-017; review [T-006](../06-tasks/T-006-Review-security-licensing.md) | R, A |
...
| F-22 | Responsive UI, light/dark/auto theme, remembered options, selection/bulk controls, toasts, version information. | Equivalent adaptive UX on all targets plus keyboard, screen-reader and large-text support; distinguish local preferences from server configuration. | M3 / [T-020](../06-tasks/T-020-Sharing-and-UX.md) | R, U |
...
| F-24 | Reverse proxy, TLS, URL prefix, host/port/IPv6, public artifact URLs, CORS, optional directory listing and robots file. | Document equivalent hosting outcomes with explicit origins and authenticated/private downloads by default; test base-path and proxy event streaming. Directory indexing must be explicit and safe. | M3 / T-021 | R, A |
| F-25 | Self-hosted amd64/arm64 containers; permissions/umask; logs; yt-dlp version/update controls including optional nightly updates. | Reproducible pinned runtime packaging, persistent volumes, redacted logs/version diagnostics, opt-in update channel with recovery/rollback rather than uncontrolled mid-job replacement. | M3 / T-021 | R, U, A |
| F-26 | Private-address rejection by default, credential-aware CORS, protected state directories. | Defense-in-depth SSRF/egress policy, redirects/DNS tests, authenticated API/artifacts, no web exposure of cookies or state. Unsafe internal-address bypass is not a normal user option. | M1–M3 / [T-009](../06-tasks/T-009-Backend-vertical-slice.md), T-021 | R, A |
