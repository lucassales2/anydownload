...
Phase D1 implements the desktop workflows for these rows by calling an installed yt-dlp, not by porting it. The task map is in [Phase 1](../00-project/Phase-1-Desktop-MeTube.md). D2 proved a direct file on each host. D3 ([Phase 3](../00-project/Phase-3-Generic-Extractor.md)) added one generic HTML subset. D4 ([Phase 4](../00-project/Phase-4-Extractor-Core-and-YouTube.md)) adds YouTube single video at single-file quality; F-02/F-03 stay open because nothing merges or converts yet. A D1, D3, or D4 pass is not the [T-022](../06-tasks/T-022-Parity-audit.md) audit. The engine underneath these workflows is tracked separately in the [yt-dlp equivalence matrix](Ytdlp-equivalence.md).
...
| F-20 | Optional per-download free-form yt-dlp options (upstream warns of command-execution risk). | **Decided:** allowlist only (owner, 2026-09-23, Q-09 / ADR-006). Free-form JSON stays disabled. Documented parity difference. | M3 / T-017; [T-006](../06-tasks/T-006-Review-security-licensing.md) | R, A |
