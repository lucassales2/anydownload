...
- **Spotify:** after the media toolkit (D5), Phase D6 resolves Spotify URLs in Kotlin with the spotDL v4.5.2 workflow, then hands each match to that same download path. YouTube Music first, then YouTube, then opt-in SoundCloud, Bandcamp, Piped, and slider.kz. Embed title, artists, album, artwork, and lyrics. save, url, meta, sync, and the user's library are in that phase ([ADR-010](../03-decisions/ADR-010-Spotdl-parity-phase.md)).
