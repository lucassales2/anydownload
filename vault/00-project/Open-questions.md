...
| Q-09 | Are security-scoped yt-dlp overrides acceptable instead of arbitrary executable options? | **Decided:** allowlist only (owner, 2026-09-23). Typed options the app builds. Free-form yt-dlp JSON stays disabled. No shell, plugins, or path escape. [ADR-006](../03-decisions/ADR-006-Local-http-engine-phase.md). | M0 / T-003, T-006 |
...
| Q-15 | What does "yt-dlp feature equivalence" mean for the Kotlin port, and how is it measured? | **Decided** (owner, 2026-09-24, [ADR-008](../03-decisions/ADR-008-Extractor-core-and-youtube-phase.md)): the core engine (info_dict/formats, format-selection language, HTTP/HLS/DASH downloaders, playlists, templates, cookies, postprocessors, typed options) plus the full extractor catalog over time. Measured in the [equivalence matrix](../01-product/Ytdlp-equivalence.md): hand-written core rows and a coverage table generated from `port/manifest.json`. Plugins, exec hooks, free-form options, and impersonation are documented differences. | D4 / [T-055](../06-tasks/T-055-Port-manifest-and-equivalence-matrix.md) |
| Q-16 | How does YouTube's JavaScript challenge run on each host? | **Decided** (owner, 2026-09-24, ADR-008): a shared `JsRuntime` port; bundled yt-dlp-ejs `0.8.0` with hash verification; Zipline QuickJS on JVM/Android/iOS pending the [T-069](../06-tasks/T-069-Ejs-bundle-and-jsruntime-port.md) spike (JavaScriptCore on iOS if it fails); optional Deno on `PATH` on desktop; the page's own JavaScript on web. `jsinterp` is not ported. | D4 / T-069, [T-071](../06-tasks/T-071-Js-runtime-desktop-android-ios.md), [T-072](../06-tasks/T-072-Js-runtime-web-page.md) |
