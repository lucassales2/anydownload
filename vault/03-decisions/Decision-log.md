...
| [ADR-007](ADR-007-Generic-extractor-phase.md) | First extractor is a generic subset; media toolkit recorded, not built | Accepted | Owner, 2026-09-23. Does not supersede ADR-004, ADR-005, or ADR-006. Tasks T-045–T-051. |

The project license is MIT as of 2026-09-21. A 2026-09-23 comparison of Python packagers and the Kotlin port is in [Client yt-dlp options](../05-research/Client-yt-dlp-options.md). It does not replace ADR-004. Owner answers the same day: Unlicense yt-dlp may be translated with notices; MeTube/NewPipe/YtDlp-kt must not be copied; Q-09 is allowlist. The per-target media toolkit and the first extractor are recorded in [ADR-007](ADR-007-Generic-extractor-phase.md). Next records after D3 should capture YouTube via yt-dlp-ejs. Use the [ADR template](../99-templates/ADR-template.md).
